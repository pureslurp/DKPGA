...
def past_results(df_merge):
    ...
    time.sleep(3)
    driver.get("https://www.golfchannel.com/tours/european-tour/2020/joburg-open")
    time.sleep(3)
    html = driver.page_source
    tables = pd.read_html(html)
    print(tables)
    dk_pastResults = tables[0]
    driver.close()
    
    #dk_pastResults = dk_pastResults[0]
    dk_pastResults.drop(['POS','MOV', 'THRU', 'O/A  OVERALL', 'R1','R2','R3','R4','ODDS', 'RND'],axis=1,inplace=True)
    
    print(dk_pastResults.info())
    print(dk_pastResults['PLAYER'])
...
def driving_accuracy(df_merge):
    ...
    driveDistScale = np.concatenate((np.linspace(5,0,len(df_merge['DriveAcc'].dropna())),np.zeros(len(df_merge)-len(df_merge['DriveAcc'].dropna()))))
...
def DK_csv_assignemnt(path, name):
    ...
    avgPointsScale = np.linspace(10, 35,len(df['AvgPointsPerGame'].dropna()))
