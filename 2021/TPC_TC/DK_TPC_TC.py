...
#draftkings csv
df = pd.read_csv('DKSalaries-TPC_TC.csv')
...
eff500 = course_df[(course_df['Yards'] < 550) & (course_df['Yards'] > 500)]['Yards'].count()
eff550 = course_df[(course_df['Yards'] < 600) & (course_df['Yards'] > 550)]['Yards'].count()
eff600 = course_df[(course_df['Yards'] < 650) & (course_df['Yards'] > 600)]['Yards'].count()

scales = [eff150,eff175,eff200,eff225,eff300,eff350,eff400,eff450,eff500,eff550,eff600]
keys = ['150-175 Eff','175-200 Eff','200-225 Eff','225-250 Eff','300-350 Eff','350-400 Eff','400-450 Eff','450-500 Eff','500-550 Eff','550-600 Eff','600-650 Eff']
...
dk_pastResults = pd.read_html('https://www.espn.com/golf/leaderboard/_/tournamentId/401155472')
