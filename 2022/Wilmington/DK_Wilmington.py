...
path = '2022/Wilmington/'
name = 'DKSalaries-Wilmington.csv'
...
df_merge = pga_odds_pga(df_merge, upper_bound=10)

df_merge.to_csv('{}/CSVs/DKData3.csv'.format(path), index = False) #optional line if you want to see data in CSV

df_merge = df_total_and_reformat(df_merge)

df_merge.to_csv('{}/CSVs/DKFinal3.csv'.format(path), index = False) #optional line if you want to see data in CSV
...
while k < 40:
    ...
    if currentIter > (mean + sigma - 4) and constraint(lineup, df_merge):
...
countPlayers.to_csv('{}CSVs/Player_Ownership3.csv'.format(path, index=False))
MaximizedLineup.to_csv('{}CSVs/Maximized_Lineups3.csv'.format(path),index=False)
