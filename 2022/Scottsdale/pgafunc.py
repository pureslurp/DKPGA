...
def past_results(df_merge, url, lowerBound=0, upperBound=5):
    ...
    pastResultsScale = np.concatenate((np.linspace(upperBound,lowerBound,len(df_merge['TOT'].dropna())),np.zeros(len(df_merge)-len(df_merge['TOT'].dropna()))))
