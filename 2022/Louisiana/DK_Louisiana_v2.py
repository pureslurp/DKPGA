...
df = drop_players_lower_than(df, 6400)
df_merge = weight_efficiencies(df, course_df)
df_merge = pga_odds_vegas_duo(df_merge)
df_merge = course_fit(df_merge)
df_merge = past_results_dyn_duo(df_merge, 'https://www.pgatour.com/tournaments/zurich-classic-of-new-orleans/past-results.html', upperBound=4.5)
df_merge = past_results_dyn_duo(df_merge, 'https://www.pgatour.com/tournaments/zurich-classic-of-new-orleans/past-results.html', upperBound=2, pr_i=1)
...
while k < 50:
    ...
    if currentIter > (mean + sigma - 10) and constraint(lineup, df_merge):
